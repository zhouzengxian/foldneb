...
| **V4.11** | 📱 **手机端全面调优**：① viewport 防缩放 + 全局 touch-action 防回弹 ② OnboardingGuide 手机端缩小（card/标题/按钮）③ PhoneApp 朋友圈手机端缩小（340→230）+ 收缩标签缩小 ④ 右下角决策推演/时间折叠按钮缩小 ⑤ 顶部布局三区不重叠（Banner → DevProgressBar → 标题栏）⑥ "播放demo" 手机端一行不换行 ⑦ title 去掉"开发进度 80%" + 防缓存 meta ⑧ AgentPanel × 按钮放大（36px）+ 侧栏宽度自适应 + z-index 全局重构（55/57/58）⑨ 朋友圈 × 关闭修复 ⑩ 决策推演面板修复（手机端隐去右侧图谱，flex column，滚动跟随打字）⑪ 星体旋转速度滑块（0.2~5x）⑫ 更多功能按钮文案 + 调试按钮更名 ⑬ 旁白 Audio 复用（修复手机端仅首句有声音） | — |
