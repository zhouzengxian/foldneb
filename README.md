...
| **V4.0+** | 📖 **思想者深度档案**：黄仁勋/马斯克首批补齐 6 类深度字段（传记/时间轴/核心思想/语录/代表作/启示）→ Modal 6 大版块带视觉分隔；graceful fallback 让 123 个无深度字段 agent 不破坏 |
| **V4.1** | 🦐 **Skill 化大版本**：商业情报分析从硬编码 prompt 升级为**用户可编辑/新增/切换的 Skill 系统**（内置「商业动态逻辑情报虾」三层框架）+ Skill 管理面板（列表/编辑/导出/重置）+ 三渠道本地留痕（项目源 + Obsidian vault `5_skill库` + 运行时导出 md） |
