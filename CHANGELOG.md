...
使用 **微软 Edge 神经网络 TTS**（晓晓 `zh-CN-XiaoxiaoNeural`，免费无需 Key）预生成 8 段高质量 MP3 旁白到 `public/narration/`。

`NebulaScene.jsx` 新增三层语音回退链：
1. **预录制 MP3**（最高质量，离线可播）
2. **浏览器 Neural TTS**（`speechSynthesis`，在线语音）
3. **浏览器基础 TTS**（终极兜底）

旁白开关统一受 `narrationEnabled` 控制，不朗读时自动静音。

### 📡 全量叙事定位升级

125 → 1000+：
- 左上角副标题：`13个星系 · 1000+位思想者 · 83亿个人生 · 1万亿个灵魂`
- README 所有营销描述改为 1000+，数据层保留「首批 125 位种子（持续扩展至 1000+）」的事实标注
- 引导页 OnboardingGuide 同步更新

### 🔧 API 全平台统一（核心架构改动）

**问题**：之前 5 个模块各维护独立的 Provider 选择变量，造成「配一次 Key 只影响一个面板」的体验割裂。

**解决**：`modelConfig.js` 新增 `getUnifiedProvider()` / `setUnifiedProvider()`，持久化到 localStorage 的 `foldneb_unified_provider` key。一次切换，全平台同步。

| 模块 | 旧机制 | 新机制 |
|------|--------|--------|
| 决策推演 | 内存变量 `currentProviderId` | `getUnifiedProvider()` |
| 时间折叠 | 继承决策推演（刷新丢失） | `getUnifiedProvider()` |
| 朋友圈/分身聊天 | 内存变量 `currentProviderId` | `getUnifiedProvider()` |
| 档案分析 | 独立 localStorage key | `getUnifiedProvider()` |

旧 API（`setDeliberationProvider`/`setTemporalProvider`/`setMomentsProvider`/`setArchiveProvider`）保留为同名别名，向后兼容。

### 🎨 UI 微调
- 左上角「搜索思想者」↔「🗂️ 十三星系」按钮对调
- 右下角「⚡ 决策推演」↔「⏳ 时间折叠」按钮对调
- 开发者工具弹出面板标题「待完善功能」→「更多功能」
...
- **顶部「这是什么 — 你的 3D 思考舱」三场景卡片**：做决策时 / 迷茫时 / 学习时，用户第一眼就能理解项目用途
...
## V2.3 — 体验打磨：打字机气泡 + 模型适配 (2026-06-13)

面向演示效果的体验打磨版本，重点修复了 GLM-5.1 thinking 模式下的解析问题，并把语录从静态文本升级为逐字打字气泡。

### 核心改动

#### 1. 打字机语录气泡
- Agent 语录从一次性显示改为**逐字浮现**的打字机气泡，配合右侧面板信息密度提升，让详情页更"有呼吸感"。

#### 2. 小米 MiMo 接入
- 多模型系统新增小米 MiMo 端点，扩展可选模型范围。

#### 3. GLM-5.1 thinking 模式适配
- 修复智谱 GLM-5.1 在 thinking 模式下返回结构导致的**推演脉络字体变形**问题，正确解析思考链输出。

#### 4. 调试与稳定性
- 推演面板新增「请求 JSON 预览 / 测试连接」入口，方便现场验证 API 配置。
- 修复多 Agent 并发推演时的 **API 限流问题**，避免批量调用被拒。

---

## V2.2 — 模型优化：DeepSeek 默认 + 智谱专属端点 (2026-06-13)

集中优化多模型 API 配置：确立默认模型、清理失效端点、为智谱 Coding Plan 接入专属端点。

### 核心改动

#### 1. DeepSeek 设为默认
- 将 DeepSeek 调整为决策推演的默认模型，并清理已失效的内置端点。

#### 2. 智谱 Coding Plan 专属端点
- 接入智谱 Coding Plan 专属端点，支持 GLM-4.7 / 4.6 / 4.6v 模型。

#### 3. 无 Key 提前拦截
- 在发起请求前检测 API Key 是否已配置，未配置时直接给出友好提示，避免无效请求和模糊报错。

---

## V2.1 — 功能集成：朋友圈 + 决策推演 + 多模型 API (2026-06-13)

从「纯 3D 可视化」升级为**有社交、有推演、有 AI 对话**的完整产品形态。本版本一次性集成三大子系统，是项目从展示工具走向交互产品的转折点。

### 核心改动

#### 1. 朋友圈社交系统（`PhoneApp.jsx`）
- 新增仿微信的手机端 UI：朋友圈动态流、通讯录、Agent 头像与昵称展示。
- Agent 按预设语料在朋友圈发帖、互动，营造"思想者们在社交"的氛围。

#### 2. 决策推演引擎（`deliberationEngine.js`）
- 新增横向决策推演：选定议题后，多位 Agent 基于各自人设展开多轮讨论，产出推演脉络与共识/分歧。
- 引擎驱动图谱节点动态生长，推演过程可视化。

#### 3. 多模型 API 系统（`modelConfig.js`）
- 统一的多 provider 配置层，支持 DeepSeek、智谱、小米等多个模型供应商，用户可自由切换。
- API Key 持久化到 localStorage，一次配置多次复用。

#### 4. CORS 代理 + 错误透传
- 接入 CORS 代理解决 GitHub Pages 静态托管下的浏览器跨域限制，让线上 Demo 也能真实调用大模型。
- API 错误信息从静默吞掉改为**透传到 UI**，用户能看到真实失败原因（超时/网络/CORS/HTTP 错误码）。

---

## V2.0 — 重大升级：125 Agent + 力导向布局 + Obsidian 关联 (2026-06-13)

从 V1.0 的基础星河原型升级为**有规模、有交互、有外部关联**的可演示产品。Agent 数量从少量扩展到 125 位，并完成视觉重构与静态部署。

### 核心改动

#### 1. 125 Agent 星体扩展（`gameData.js`）
- Agent 数据从少量扩展到 **125 位**横跨科技/哲学/商业/历史的思想者，每颗星体承载一位人物的档案。

#### 2. 力导向布局（`NebulaScene.jsx`）
- 采用力导向算法自动排布星体，相关 Agent 自然聚类，避免手动定位，让星河结构随数据自适应。

#### 3. FoldNeb 折叠星云视觉重构
- 整体视觉重构为"折叠星云"主题：深空背景、发光星体、粒子效果，确立项目标志性视觉语言。

#### 4. Obsidian 关联跳转
- 支持从星体一键跳转到 Obsidian 对应笔记（`obsidian://open` URI 协议），打通 3D 星河与本地知识库。

#### 5. GitHub Pages 部署
- 完成静态构建与 GitHub Pages 部署配置，项目可在线访问，方便演示。

---

## V1.0 — 完整构建：3D 星河可视化基座 (2026-06-13)

黑客松起步版本，完成 6 个 Phase 的全部基础建设，搭起 3D 星河可视化的技术基座。

### 核心内容

- **基础 3D 星河可视化**：Three.js 场景搭建、深空背景、星体渲染管线。
- **星体渲染**：每颗 Agent 以发光星体形态呈现，带光晕与脉动效果。
- **相机交互**：支持旋转、缩放、平移，点击星体聚焦查看详情。
- 项目方向确定：FoldNeb 折叠星云——把思想者化为星空。

---

## 初始化 — 黑客松竞赛项目立项 (2026-06-13)

确定 FoldNeb 折叠星云方向，完成项目脚手架与依赖初始化（React + Three.js + Zustand + GSAP 技术栈）。
